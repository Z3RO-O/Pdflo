...
const SEOHead: React.FC<SEOHeadProps> = ({ task }) => {
  useEffect(() => {
    if (task) {
      ...
      document.title = 'Pdflo - Free PDF Converter & Editor | Convert PDF to Word, Excel, PowerPoint'
      updateMetaTag('description', 'Free online PDF converter and editor. Convert PDF to Word, Excel, PowerPoint, images and more. Merge, split, compress, and protect PDFs. 27+ professional PDF tools in one place.')
  ...
  const generateKeywords = (task: ConversionTask): string => {
    ...
    if (task.id.includes('excel')) baseKeywords.push('Excel', 'XLSX', 'XLS', 'spreadsheet')
    if (task.id.includes('powerpoint')) baseKeywords.push('PowerPoint', 'PPTX', 'PPT', 'presentation')
    if (task.id.includes('image')) baseKeywords.push('image', 'JPG', 'PNG', 'photo')
    if (task.id.includes('merge')) baseKeywords.push('merge', 'combine', 'join')
    if (task.id.includes('split')) baseKeywords.push('split', 'extract', 'separate')
    if (task.id.includes('compress')) baseKeywords.push('compress', 'reduce size', 'optimize')
  ...
  const updateStructuredData = (task: ConversionTask) => {
    ...
    const existingScript = document.querySelector('script[type="application/ld+json"]#tool-schema')
    if (existingScript) {
