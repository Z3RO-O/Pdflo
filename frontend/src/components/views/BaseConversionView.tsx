...
const BaseConversionView: React.FC<BaseConversionViewProps> = ({
  ...
  const setUploadedFiles = parentUploadedFiles
    ? () => {}
  ...
  const optionsWithErrors = task.options?.map((opt) => {
    if (
      error &&
      (error.toLowerCase().includes(opt.label.toLowerCase()) ||
        error.toLowerCase().includes(opt.id.toLowerCase()))
    ) {
      return { ...opt, error }
    }
    return opt
          ...
          <Button
            ...
            disabled={
              ...
              (task.requiresFileUpload &&
                ...
                !children)
            } // Disable if no files and no special child input
            size="lg"
            className="w-full sm:w-auto grow"
          >
            {isConverting
              ? 'Converting...'
              : `Convert to ${task.isEditingTool ? 'Apply Changes' : task.supportedOutputFormats?.[0]?.label || 'Output'}`}
          </Button>
          <Button
          ...
          <div className="mt-6 md:mt-8 p-4 md:p-6 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded-lg">
            ...
            <ul className="space-y-2">
              {processedFiles.map((file) => (
                <li
                  key={file.id}
                  className="p-3 bg-white dark:bg-neutral-800 rounded-md shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0"
                >
                  <span
