...
const ImageEditDialog: React.FC<ImageEditDialogProps> = ({
  ...
  const applyCrop = () => {
    if (crop && imgRef && imgRef.naturalWidth > 0 && imgRef.naturalHeight > 0) {
      // Check natural dims
      ...
      console.warn(
        'Cannot apply crop: Crop data or image reference is invalid or image not loaded.',
      )
    }
  }
  ...
  const applyEdits = () => {
    ...
    img.onload = () => {
      ...
      switch (filter) {
        case 'grayscale':
          currentFilterString += ' grayscale(100%)'
          break
        case 'sepia':
          currentFilterString += ' sepia(100%)'
          break
        case 'blur':
          currentFilterString += ' blur(2px)'
          break
        case 'saturate':
          currentFilterString += ' saturate(200%)'
          break
      }
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, p: 2 }}>
          ...
          >
            ...
            <Tooltip title={isCropping ? 'Cancel Crop' : 'Crop Tool'}>
              <IconButton
      ...
      <DialogActions sx={{ flexWrap: 'wrap', justifyContent: 'center', p: 2 }}>
        ...
        <Button onClick={applyEdits} variant="contained" color="primary">
          Save Changes
        </Button>
      </DialogActions>
    </Dialog>
...
const MuiImageToPdfTool: React.FC = () => {
  ...
  const handleEditSave = async (editedImageBlobUrl: string, index: number) => {
    try {
      ...
      setPreviews((prev) => {
        ...
        newPreviews[index] = editedImageBlobUrl
        return newPreviews
      })
      setFiles((prev) => {
        const newFiles = [...prev]
  ...
  const handleDrop = useCallback(
    async (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (unsupportedFiles.length > 0) {
        setNotification({
          type: 'error',
          message: `Unsupported file type(s): ${unsupportedFiles.map((f) => f.name).join(', ')}. Please upload JPG, PNG, WEBP, HEIC, or GIF.`,
        })
        return
      }
      setError(null)
  ...
  const handleDragOver = (
    ...
    const draggedOriginalPreview = newOriginalPreviews.splice(
      draggedIndex,
      1,
    )[0]
  ...
  const handleFiles = useCallback(async (newFilesArray: File[]) => {
    const heicFiles = newFilesArray.filter(
      (file) =>
    ...
    for (const file of newFilesArray) {
      try {
        ...
        if (
          ...
          if (heicFiles.length > 0) {
            setHeicConversionProgress(
              (heicProcessedCount / heicFiles.length) * 100,
            )
          }
        }
        ...
        setError(
          `Error processing file ${file.name}: ${fileError.message}. Skipped.`,
        )
      }
    }
  ...
  const removeFile = useCallback(
    (index: number) => {
      ...
      if (
        originalPreviewToRemove &&
        originalPreviewToRemove.startsWith('blob:')
      ) {
  ...
  const convertToPdf = async (compressionLevel = 'normal') => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const availableWidth = pageWidth - 2 * borderSpacing
        const availableHeight = pageHeight - 2 * borderSpacing
        let finalWidth, finalHeight
        if (maintainAspectRatio) {
          const ratio = Math.min(
            availableWidth / compressedImg.naturalWidth,
            availableHeight / compressedImg.naturalHeight,
          )
          finalWidth = compressedImg.naturalWidth * ratio
          finalHeight = compressedImg.naturalHeight * ratio
        } else {
        ...
        if (i > 0)
          pdf.addPage(pageSize, orientation as 'portrait' | 'landscape')
        ...
        if (addHeaderFooter && headerText.trim() !== '') {
          pdf.setFontSize(headerFontSize)
          pdf.text(headerText, pageWidth / 2, 10, { align: 'center' })
        }
        if (addHeaderFooter && footerText.trim() !== '') {
        ...
        if (pageNumbers) {
          ...
          let pnX,
            pnY,
            pnAlign: 'left' | 'center' | 'right' | 'justify' | undefined
        ...
        if (addWatermark && watermark.trim() !== '') {
          ...
          const watermarkTextOptions: any = {
            angle: -45,
            align: 'center',
            opacity: watermarkOpacity,
          } // jsPDF types might not have opacity directly here
        ...
        if (addSignature && signatureText.trim() !== '') {
          ...
          let sigX,
            sigY,
            sigAlign: 'left' | 'center' | 'right' | 'justify' | undefined
          const margin = 10
          switch (signaturePosition) {
            case 'bottom-center':
              sigX = pageWidth / 2
              sigY = pageHeight - margin
              sigAlign = 'center'
              break
            case 'bottom-left':
              sigX = margin
      ...
      if (compressionLevel === 'compressed')
        filename = 'image-to-pdf-maker-compressed.pdf'
      else if (compressionLevel === 'ultra')
        filename = 'image-to-pdf-maker-email.pdf'
      pdf.save(filename)
      setTimeout(() => {
        setConverting(false)
      }, 1000)
      ...
      >
        ...
        <Container maxWidth="lg" sx={{ flex: 1, py: { xs: 1, sm: 2, md: 3 } }}>
          <Box sx={{ my: { xs: 2, md: 3 } }}>
            ...
            >
              Convert images (PNG, JPG, WEBP, HEIC, GIF) to PDF with advanced
              options.
            </Typography>
            ...
            {heicConverting && (
              <Box sx={{ mt: 3, textAlign: 'center' }}>
                ...
                <Typography variant="h6" color="primary" gutterBottom>
                  Converting HEIC Files... ({Math.round(heicConversionProgress)}
                  %)
                </Typography>
                <StyledLinearProgress
                  variant="determinate"
                  value={heicConversionProgress}
              ...
              <Paper sx={{ mt: 3, p: { xs: 2, md: 3 } }}>
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    />
                    <FormLabel>Border Spacing: {borderSpacing}mm</FormLabel>
                    <Slider
                      value={borderSpacing}
                      onChange={(_, val) => setBorderSpacing(val as number)}
                      min={0}
                      max={30}
                      step={1}
                      marks
                    ...
                    {addHeaderFooter && (
                      <>
                        ...
                        <Slider
                          ...
                          onChange={(_, v) => setHeaderFontSize(v as number)}
                          min={8}
                          max={24}
                          step={1}
                          marks
                        ...
                        />
                        <FormLabel>
                          Footer Font Size: {footerFontSize}pt
                        </FormLabel>
                        <Slider
                          value={footerFontSize}
                          onChange={(_, v) => setFooterFontSize(v as number)}
                          min={8}
                          max={24}
                          step={1}
                          marks
                    ...
                    {addWatermark && (
                      <>
                            ...
                            <Tooltip title={color.name} key={color.value}>
                              <Button
                                ...
                                sx={{
                                  ...
                                  boxShadow:
                                    watermarkColor === color.value
                                      ? `0 0 8px ${alpha(color.value, 0.7)}`
                                      : 'none',
                                  '&:hover': {
                                    bgcolor: alpha(color.value, 0.8),
                        ...
                        <Slider
                          ...
                          sx={{ mb: 1 }}
                        />
                        <FormLabel>Font Size: {watermarkFontSize}px</FormLabel>
                        <Slider
                          value={watermarkFontSize}
                          onChange={(_, v) => setWatermarkFontSize(v as number)}
                          min={30}
                          max={100}
                          step={1}
                          marks
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    {addSignature && (
                      <>
                        ...
                        <FormLabel>Font Size: {signatureFontSize}pt</FormLabel>
                        <Slider
                          value={signatureFontSize}
                          onChange={(_, v) => setSignatureFontSize(v as number)}
                          min={8}
                          max={24}
                          step={1}
                          marks
            ...
            {previews.length > 0 && (
              <>
                <Typography
                  ...
                  color="text.secondary"
                  align="center"
                  sx={{ mt: 3, mb: 1, fontStyle: 'italic' }}
                >
                  Drag images to reorder. Click edit icon on image for more
                  options.
                </Typography>
                <Grid
                  container
                ...
                >
                  {previews.map((preview, index) => (
                    <Grid
                      ...
                      sm={6}
                      md={4}
                      key={preview + index + files[index]?.name}
                      draggable
                    ...
                    >
                      <ImagePreviewContainer>
                        <Box sx={{ position: 'relative' }}>
                          <ImagePreview
                            src={preview}
                            alt={`${files[index]?.name || `Image ${index + 1}`} preview`}
                            draggable={false}
                          />
                          <ActionOverlay
              ...
              >
                ...
                >
                  <Button
                    ...
                    size="large"
                    sx={{ flexGrow: 1, minWidth: { xs: '150px', sm: '200px' } }}
                  >
                    Preview PDF
                  </Button>
                  <Button
                    variant="contained"
                    onClick={() => convertToPdf('normal')}
                    startIcon={<PdfIcon />}
                    size="large"
                    sx={{ flexGrow: 1, minWidth: { xs: '150px', sm: '200px' } }}
                  >
                ...
                >
                  <Button
                    variant="contained"
                    onClick={() => convertToPdf('compressed')}
                    startIcon={<PdfIcon />}
                    size="large"
                    color="primary"
                    sx={{
            ...
            >
              ...
              >
                ...
                {!previewLoading && !previewUrl && error && (
                  <Box
                    sx={{
                      display: 'flex',
                      alignItems: 'center',
            ...
            {editingImageIndex !== null &&
              previews[editingImageIndex] &&
              originalPreviews[editingImageIndex] && (
                <ImageEditDialog
                  open={editDialogOpen}
                  onClose={handleEditDialogClose}
                  image={previews[editingImageIndex]}
                  onSave={handleEditSave}
                  index={editingImageIndex}
                  originalImage={originalPreviews[editingImageIndex]}
                />
              )}
