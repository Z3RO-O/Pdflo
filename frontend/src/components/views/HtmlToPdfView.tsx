...
const Card = styled.div.withConfig({
  ...
  border-radius: 18px;
  box-shadow: 0 4px 32px 0 rgba(60, 72, 100, 0.1);
...
const TabButton = styled.button.withConfig({
  ...
  box-shadow: ${({ active }) =>
    active ? '0 2px 12px 0 rgba(37,99,235,0.10)' : 'none'};
  cursor: pointer;
  border-bottom: ${({ active }) =>
    active
      ? '2.5px solid var(--primary-color, #2563eb)'
...
const HtmlToPdfView: React.FC<HtmlToPdfViewProps> = ({ task }) => {
  ...
  const handleFilesUpload = (files: UploadedFile[]) => {
    ...
    if (
      !acceptedMimeTypes.includes(file.file.type) &&
      !acceptedExtensions.includes(fileExtension)
    ) {
      setError(
        `File "${file.file.name}" is not a supported HTML file. Please upload .html or .htm files.`,
      )
      setUploadedFiles([])
    } else {
    ...
    <PageBackground darkMode={darkMode}>
      <Card darkMode={darkMode}>
        ...
        <ErrorMessage
          darkMode={darkMode}
          style={{
            background: darkMode ? '#3b2f1a' : '#fffbe6',
            color: darkMode ? '#fde68a' : '#b45309',
            border: darkMode ? '1.5px solid #fde68a' : '1.5px solid #fde68a',
            marginBottom: '1.5rem',
            fontWeight: 500,
          }}
        >
          <AlertCircle
            className="w-24 h-24"
            style={{ color: darkMode ? '#fde68a' : '#f59e42', marginRight: 5 }}
          />
          Note: Only static HTML files and code (without external scripts or
          dynamic content) will render perfectly. Dynamic HTML (with JavaScript,
          external CSS, or interactive features) may not be converted correctly.
        </ErrorMessage>
        <TabContainer>
          <TabButton
            active={mode === 'upload'}
