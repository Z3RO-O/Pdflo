...
const OrganizePdfView: React.FC = () => {
  ...
  const validatePageInputs = (
    ...
    if (options.page_order && totalPages) {
      const pageNumbers = options.page_order
        .split(',')
        .map((s: string) => s.trim())
      for (const pageStr of pageNumbers) {
        ...
        if (isNaN(pageNum) || pageNum < 1 || pageNum > totalPages) {
          errors.push(
            `Page ${pageNum} does not exist (PDF has ${totalPages} pages)`,
          )
          break
        }
      }
    ...
    if (options.rotate_pages && totalPages) {
      const rotations = options.rotate_pages
        .split(',')
        .map((s: string) => s.trim())
      for (const rotation of rotations) {
        if (!rotation.includes(':')) {
          errors.push(
            'Invalid rotation format. Use "page:angle" (e.g., 2:90,3:180)',
          )
          break
        }
        ...
        if (isNaN(pageNum) || pageNum < 1 || pageNum > totalPages) {
          errors.push(
            `Page ${pageNum} does not exist for rotation (PDF has ${totalPages} pages)`,
          )
          break
        }
        ...
        if (isNaN(angle) || ![90, 180, 270].includes(angle)) {
          errors.push(
            `Invalid rotation angle ${angle}. Use 90, 180, or 270 degrees`,
          )
          break
        }
      }
    ...
    if (options.delete_pages && totalPages) {
      const pageNumbers = options.delete_pages
        .split(',')
        .map((s: string) => s.trim())
      for (const pageStr of pageNumbers) {
        const pageNum = parseInt(pageStr)
        if (isNaN(pageNum) || pageNum < 1 || pageNum > totalPages) {
          errors.push(
            `Page ${pageNum} does not exist for deletion (PDF has ${totalPages} pages)`,
          )
          break
        }
      }
  ...
  const handlePageOperationsChange = useCallback(
    (operations: {
      pageOrder: string
      rotatePages: string
      deletePages: string
    ...
    >
      {isLoadingPageCount && (
        ...
        >
          ...
          >
            Please wait while we analyze your PDF file to show organization
            options.
          </Typography>
        </Box>
      )}
