...
const PdfToExcelView: React.FC<PdfToExcelViewProps> = ({
  ...
  const renderPage = async (pageNum: number, doc?: PDFDocumentProxy) => {
    ...
    try {
      ...
      console.log(
        `Page ${pageNum} rendered successfully at ${Math.round(scale * 100)}%`,
      )
    } catch (error) {
  ...
  const handleConvert = async () => {
    ...
    const finalPageSelection =
      pageSelection === 'all' ? generatePageSelection() : pageSelection
  ...
  const validatePageSelection = (selection: string): boolean => {
    ...
    const invalidPages = Array.from(pages).filter(
      (page) => page < 1 || page > maxPage,
    )

    if (invalidPages.length > 0) {
      setPageSelectionError(
        `Invalid page(s): ${invalidPages.join(', ')}. PDF has ${maxPage} page${maxPage !== 1 ? 's' : ''}.`,
      )
      return false
    }
    ...
    >
      ...
      {uploadedFile && previewLoaded && !showPreview && (
        ...
        >
          <CardContent sx={{ p: 3, textAlign: 'center' }}>
            ...
            >
              Your PDF has been loaded successfully. Click the button below to
              preview it.
            </Typography>
      ...
      >
        <CardContent>
          ...
          <TextField
            ...
            helperText={
              pageSelectionError ||
              `Specify page numbers or ranges (e.g., 1, 3-5, 7). Leave empty or type 'all' for all pages.${totalPages > 0 ? ` PDF has ${totalPages} page${totalPages !== 1 ? 's' : ''}.` : ''}`
            }
            ...
            sx={{
              ...
              '& .MuiFormHelperText-root': {
                color: pageSelectionError
                  ? 'var(--error-color)'
                  : 'var(--text-secondary)',
              },
              '& .MuiOutlinedInput-root': {
                '& fieldset': {
                  borderColor: pageSelectionError
                    ? 'var(--error-color)'
                    : 'var(--border-color)',
                },
                '&:hover fieldset': {
          ...
          {selectedTables.size > 0 && (
            ...
            >
              {selectedTables.size} table(s) selected from{' '}
              {
                Array.from(
                  new Set(
                    Array.from(selectedTables).map((key) => key.split('-')[0]),
                  ),
                ).length
              }{' '}
              page(s)
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center' }}>
        <Button
          ...
          size="large"
          startIcon={
            isConverting ? <CircularProgress size={20} /> : <DownloadIcon />
          }
