...
const MergePdfView: React.FC<MergePdfViewProps> = ({ task }) => {
  ...
  const performMergeConversion = async (
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ error: 'Failed to merge PDFs.' }))
  ...
  const renderReorderableFileList = () => {
    ...
    return (
      <div className="mt-4">
        <h4 className="text-md font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
