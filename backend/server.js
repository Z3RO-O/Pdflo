    ...
    async (error, stdout, stderr) => {
      try {
        ...
        let pdfFile = files.find(
          (f) => f.endsWith(".pdf") && f.startsWith(baseName),
        );
        if (!pdfFile) {
...
app.post("/convert/word-to-pdf", upload.single("file"), async (req, res) => {
  ...
  if (![".doc", ".docx"].includes(ext.toLowerCase())) {
    ...
    return res
      .status(400)
      .json({ error: "Only .doc and .docx files are supported" });
  }
...
app.post("/convert/excel-to-pdf", upload.single("file"), async (req, res) => {
  ...
  if (![".xls", ".xlsx"].includes(ext.toLowerCase())) {
    ...
    return res
      .status(400)
      .json({ error: "Only .xls and .xlsx files are supported" });
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (![".ppt", ".pptx"].includes(ext.toLowerCase())) {
      ...
      return res
        .status(400)
        .json({ error: "Only .ppt and .pptx files are supported" });
    }
    convertWithLibreOffice(req.file.path, res, req.file.originalname);
  },
);
...
app.post("/convert/html-to-pdf", upload.single("file"), async (req, res) => {
  ...
  if (req.file) {
    ...
    if (![".html", ".htm"].includes(ext.toLowerCase())) {
      ...
      return res
        .status(400)
        .json({ error: "Only .html and .htm files are supported" });
    }
    ...
    try {
      ...
      if (tempFilePath)
        await fs
          .unlink(tempFilePath)
          .catch((err) =>
            console.error("Failed to delete temp file for URL:", err),
          );
      return res
        .status(500)
    ...
    try {
      ...
      if (tempFilePath)
        await fs
          .unlink(tempFilePath)
          .catch((err) =>
            console.error("Failed to delete temp file for content:", err),
          );
      return res
        .status(500)
        .json({
  ...
  return res
    ...
    .json({
      error:
        "No input provided. Please upload a file, provide a URL, or paste HTML content.",
    });
});
  ...
  try {
    ...
    calibreProcess.on("close", async (code) => {
      try {
        ...
        if (code !== 0) {
          console.error(
            "[EPUB2PDF] Calibre conversion failed with code:",
            code,
          );
        ...
        try {
          ...
          return res
            .status(500)
            .json({ error: "PDF not generated by Calibre." });
        }

        console.log(
          "[EPUB2PDF] Conversion successful, sending PDF:",
          outputPdf,
        );
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader(
    ...
    calibreProcess.on("timeout", () => {
      ...
      if (!res.headersSent)
        res
          .status(500)
          .json({
            error: "Conversion timed out. Please try with a smaller file.",
          });
    });
  } catch (err) {
    ...
    if (renamedInputPath) {
      await fs
        .unlink(renamedInputPath)
        .catch((err) =>
          console.error("[EPUB2PDF] Failed to delete renamed input file:", err),
        );
    }
    if (uploadedFilePath) {
      await fs
        ...
        .catch((err) =>
          console.error("[EPUB2PDF] Failed to delete uploaded file:", err),
        );
    }
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                console.log(
                  "[PDF2WORD] Conversion successful - DOCX file sent",
                );
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error(
                    "[PDF2WORD] Failed to delete output file:",
                    err,
                  ),
                );
                resolve();
          ...
          if (
            stderr.includes("pdf2docx") ||
            stderr.includes("No module named")
          ) {
            reject(
              new Error(
                "PDF to Word conversion requires Python libraries. Please install pdf2docx by running: cd server && source pdf_converter_env/bin/activate && pip install pdf2docx",
              ),
            );
          } else {
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                console.log(
                  "[PDF2EXCEL] Conversion successful - Excel file sent",
                );
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error(
                    "[PDF2EXCEL] Failed to delete output file:",
                    err,
                  ),
                );
                resolve();
          ...
          console.error(
            "[PDF2EXCEL] Python conversion failed with code:",
            code,
          );
          ...
          // Check if it's a missing library error
          if (
            stderr.includes("pdfplumber") ||
            stderr.includes("pandas") ||
            stderr.includes("openpyxl") ||
            stderr.includes("No module named")
          ) {
            reject(
              new Error(
                "PDF to Excel conversion requires Python libraries. Please install required packages by running: cd server && source pdf_converter_env/bin/activate && pip install pdfplumber pandas openpyxl",
              ),
            );
          } else {
            reject(new Error(`Python conversion failed: ${stderr}`));
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF to Excel conversion timed out. Please try with a smaller file or fewer pages.",
          ),
        );
      });
    });
...
app.post("/convert/pdf-to-word", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    tempFilePath = path.join(
      tempDir,
      `temp_${Date.now()}_${path.basename(req.file.originalname)}`,
    );
    ...
    res
      .status(500)
      .json({ error: "PDF to Word conversion failed", details: error.message });
  } finally {
...
app.post("/convert/pdf-to-excel", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const stableFilePath = await createStableFileCopy(
      req.file.path,
      req.file.originalname,
    );
    ...
    await fs
      .unlink(req.file.path)
      .catch((err) =>
        console.error("[PDF2EXCEL] Failed to delete uploaded file:", err),
      );
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                console.log(
                  "[PDF2PPT] Conversion successful - PowerPoint file sent",
                );
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error("[PDF2PPT] Failed to delete output file:", err),
                );
                resolve();
              }
            });
          ...
          // Check if it's a missing library error
          if (
            stderr.includes("pdf2image") ||
            stderr.includes("python-pptx") ||
            stderr.includes("PIL") ||
            stderr.includes("No module named")
          ) {
            reject(
              new Error(
                "PDF to PowerPoint conversion requires Python libraries. Please install required packages by running: cd server && source pdf_converter_env/bin/activate && pip install pdf2image python-pptx Pillow",
              ),
            );
          } else {
            reject(new Error(`Python conversion failed: ${stderr}`));
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF to PowerPoint conversion timed out. Please try with a smaller file.",
          ),
        );
      });
    });
  ...
  async (req, res) => {
    ...
    try {
      ...
      await fs
        .unlink(req.file.path)
        .catch((err) =>
          console.error("[PDF2PPT] Failed to delete uploaded file:", err),
        );
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                console.log(
                  "[PDF2TEXT] Conversion successful - Text file sent",
                );
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error(
                    "[PDF2TEXT] Failed to delete output file:",
                    err,
                  ),
                );
                resolve();
          ...
          if (
            ...
            reject(
              new Error(
                "PDF to Text conversion requires Python libraries. Please install required packages by running: cd server && source pdf_converter_env/bin/activate && pip install pdfplumber",
              ),
            );
          } else {
            reject(new Error(`Python conversion failed: ${stderr}`));
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF to Text conversion timed out. Please try with a smaller file.",
          ),
        );
      });
    });
...
app.post("/convert/pdf-to-text", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    await fs
      .unlink(req.file.path)
      .catch((err) =>
        console.error("[PDF2TEXT] Failed to delete uploaded file:", err),
      );
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                console.log(
                  "[PDF2HTML] Conversion successful - HTML file sent",
                );
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error(
                    "[PDF2HTML] Failed to delete output file:",
                    err,
                  ),
                );
                resolve();
          ...
          if (
            ...
            reject(
              new Error(
                "PDF to HTML conversion requires Python libraries. Please install required packages by running: cd server && source pdf_converter_env/bin/activate && pip install pdfplumber pdf2image pytesseract",
              ),
            );
          } else {
            reject(new Error(`Python conversion failed: ${stderr}`));
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF to HTML conversion timed out. Please try with a smaller file.",
          ),
        );
      });
    });
...
app.post("/convert/pdf-to-html", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    await fs
      .unlink(req.file.path)
      .catch((err) =>
        console.error("[PDF2HTML] Failed to delete uploaded file:", err),
      );
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                console.log(
                  "[PDF2EPUB] Conversion successful - EPUB file sent",
                );
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error(
                    "[PDF2EPUB] Failed to delete output file:",
                    err,
                  ),
                );
                resolve();
          ...
          // Check if it's a missing library error
          if (
            stderr.includes("pypdf2") ||
            stderr.includes("ebooklib") ||
            stderr.includes("No module named")
          ) {
            reject(
              new Error(
                "PDF to EPUB conversion requires Python libraries. Please install required packages by running: cd server && source pdf_converter_env/bin/activate && pip install pypdf2 ebooklib",
              ),
            );
          } else {
            reject(new Error(`Python conversion failed: ${stderr}`));
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF to EPUB conversion timed out. Please try with a smaller file.",
          ),
        );
      });
    });
...
app.post("/convert/pdf-to-epub", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    await fs
      .unlink(req.file.path)
      .catch((err) =>
        console.error("[PDF2EPUB] Failed to delete uploaded file:", err),
      );
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error("[PDF2RTF] Failed to delete output file:", err),
                );
                resolve();
              }
            });
          ...
          if (stderr.includes("pypdf2") || stderr.includes("No module named")) {
            reject(
              new Error(
                "PDF to RTF conversion requires Python libraries. Please install required packages by running: cd server && source pdf_converter_env/bin/activate && pip install pypdf2",
              ),
            );
          } else {
            reject(new Error(`Python conversion failed: ${stderr}`));
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF to RTF conversion timed out. Please try with a smaller file.",
          ),
        );
      });
    });
...
app.post("/convert/pdf-to-rtf", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    await fs
      .unlink(req.file.path)
      .catch((err) =>
        console.error("[PDF2RTF] Failed to delete uploaded file:", err),
      );
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error("[PDF2SVG] Failed to delete output file:", err),
                );
                resolve();
              }
            });
          ...
          if (
            ...
            reject(
              new Error(
                "PDF to SVG conversion requires Python libraries. Please install required packages by running: cd server && source pdf_converter_env/bin/activate && pip install pdf2image",
              ),
            );
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF to SVG conversion timed out. Please try with a smaller file or fewer pages.",
          ),
        );
      });
    });
...
app.post("/convert/pdf-to-svg", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    await fs
      .unlink(req.file.path)
      .catch((err) =>
        console.error("[PDF2SVG] Failed to delete uploaded file:", err),
      );
  ...
  try {
    ...
    for (const file of req.files) {
      ...
      if (path.extname(file.originalname).toLowerCase() !== ".pdf") {
        throw new Error(
          `Invalid file type: ${file.originalname}. Only PDFs are allowed.`,
        );
      }
      const stablePath = await createStableFileCopy(
        file.path,
        file.originalname,
      );
    ...
    console.log(
      `[MERGE_PDF] Executing Python script: ${pythonScriptPath} with args: ${args.join(" ")}`,
    );
    ...
    pythonProcess.on("timeout", () => {
      ...
      res
        .status(500)
        .json({ error: "PDF merge timed out. Please try with smaller files." });
      return;
    });
    ...
    pythonProcess.on("close", async (code) => {
      ...
      if (code === 0) {
        try {
          ...
          fileStream.on("close", async () => {
            ...
            await fs
              .unlink(outputPath)
              .catch((e) => console.error("Failed to delete output file:", e));
          });
        } catch (err) {
          ...
          res
            .status(500)
            .json({
              error: "Failed to create or send merged file.",
        ...
        res
          .status(500)
          .json({ error: "PDF merge failed.", details: stderr || stdout });
      }
      ...
      for (const p of stableFilePaths) {
        await fs
          .unlink(p)
          .catch((e) => console.error("Failed to delete stable temp file:", e));
      }
    });
  } catch (err) {
...
const convertPdfAndZip = async (req, res, conversionType) => {
  ...
  if (path.extname(req.file.originalname).toLowerCase() !== ".pdf") {
    ...
    return res
      .status(400)
      .json({ error: "Only PDF files are supported for this operation." });
  }

  let stableFilePath;
  try {
    stableFilePath = await createStableFileCopy(
      req.file.path,
      req.file.originalname,
    );
    ...
    pythonProcess.on("timeout", () => {
      console.error(
        `[${conversionType}] Python process timed out after 4 minutes`,
      );
      pythonProcess.kill("SIGKILL");
      res
        .status(500)
        .json({
          error: `${conversionType} conversion timed out. Please try with a smaller file.`,
        });
      return;
    });
    ...
    pythonProcess.on("close", async (code) => {
      ...
      if (code === 0) {
        try {
          ...
          fileStream.pipe(res);
          fileStream.on("close", () => {
            fs.unlink(outputPath).catch((e) =>
              console.error("Failed to delete output ZIP:", e),
            );
          });
        } catch (err) {
          ...
          res
            .status(500)
            .json({
              error: "Failed to create or send ZIP file.",
              details: stderr || stdout,
            });
        }
      } else {
        res
          .status(500)
          .json({
            error: `Python script failed for ${conversionType}.`,
            details: stderr || stdout,
          });
      }
      // Cleanup stable file
      fs.unlink(stableFilePath).catch((e) =>
        console.error("Failed to delete stable temp file:", e),
      );
    });
  } catch (err) {
    ...
    if (stableFilePath) {
      fs.unlink(stableFilePath).catch((e) =>
        console.error("Failed to delete stable temp file on error:", e),
      );
    }
  } finally {
...
const convertPdfCompress = async (
  filePath,
  res,
  originalName = "document.pdf",
  options = {},
  ...
  try {
    ...
    const outputFileName =
      path.basename(originalName, ".pdf") + "_compressed.pdf";
    const absoluteOutputPath = path.resolve(uploadsDir, outputFileName);
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                console.log(
                  "[PDFCOMPRESS] Compression successful - PDF file sent",
                );
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error(
                    "[PDFCOMPRESS] Failed to delete output file:",
                    err,
                  ),
                );
                resolve();
            ...
            reject(
              new Error("Compression completed but output file not found"),
            );
          }
        } else {
          console.error(
            "[PDFCOMPRESS] Python compression failed with code:",
            code,
          );
          ...
          // Check if it's a missing library error
          if (
            stderr.includes("ghostscript") ||
            stderr.includes("No module named")
          ) {
            reject(
              new Error(
                "PDF compression requires Ghostscript. Please install it: brew install ghostscript",
              ),
            );
          } else {
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF compression timed out. Please try with a smaller file or different compression settings.",
          ),
        );
      });
    });
...
app.post("/convert/compress-pdf", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    await fs
      .unlink(req.file.path)
      .catch((err) =>
        console.error("[PDFCOMPRESS] Failed to delete uploaded file:", err),
      );
...
const convertPdfProtect = async (
  ...
  try {
    ...
    const absoluteInputPath = path.resolve(filePath);
    const outputFileName =
      path.basename(originalName, ".pdf") + "_protected.pdf";
    const absoluteOutputPath = path.resolve(uploadsDir, outputFileName);
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                console.log(
                  "[PDFPROTECT] Protection successful - PDF file sent",
                );
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error(
                    "[PDFPROTECT] Failed to delete output file:",
                    err,
                  ),
                );
                resolve();
          ...
          console.error(
            "[PDFPROTECT] Python protection failed with code:",
            code,
          );
          ...
          if (stderr.includes("PyPDF2") || stderr.includes("No module named")) {
            reject(
              new Error(
                "PDF protection requires PyPDF2. Please install it: pip install PyPDF2",
              ),
            );
          } else {
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF protection timed out. Please try with a smaller file.",
          ),
        );
      });
    });
  ...
  async (req, res) => {
    ...
    try {
      ...
      await fs
        .unlink(req.file.path)
        .catch((err) =>
          console.error("[PDFPROTECT] Failed to delete uploaded file:", err),
        );
...
const convertPdfOrganize = async (
  filePath,
  res,
  originalName = "document.pdf",
  options = {},
  ...
  try {
    ...
    const absoluteInputPath = path.resolve(filePath);
    const outputFileName =
      path.basename(originalName, ".pdf") + "_organized.pdf";
    const absoluteOutputPath = path.resolve(uploadsDir, outputFileName);
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on("close", async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            res.sendFile(absoluteOutputPath, (err) => {
              if (err) {
                ...
                console.log(
                  "[PDFORGANIZE] Page organization successful - PDF file sent",
                );
                // Clean up the output file after sending
                fs.unlink(absoluteOutputPath).catch((err) =>
                  console.error(
                    "[PDFORGANIZE] Failed to delete output file:",
                    err,
                  ),
                );
                resolve();
            ...
            reject(
              new Error(
                "Page organization completed but output file not found",
              ),
            );
          }
        } else {
          console.error(
            "[PDFORGANIZE] Python page organization failed with code:",
            code,
          );
          ...
          if (stderr.includes("PyPDF2") || stderr.includes("No module named")) {
            reject(
              new Error(
                "PDF page organization requires PyPDF2. Please install it: pip install PyPDF2",
              ),
            );
          } else {
      ...
      pythonProcess.on("timeout", () => {
        ...
        reject(
          new Error(
            "PDF page organization timed out. Please try with a smaller file.",
          ),
        );
      });
    });
  ...
  async (req, res) => {
    ...
    try {
      ...
      await fs
        .unlink(req.file.path)
        .catch((err) =>
          console.error("[PDFORGANIZE] Failed to delete uploaded file:", err),
        );
...
app.post("/convert/ocr-pdf", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const result = await performOcrConversion(
      stableFilePath,
      outputPath,
      options,
    );
    ...
    await fs
      .unlink(req.file.path)
      .catch((err) =>
        console.error("[OCR_PDF] Failed to delete uploaded file:", err),
      );
...
app.post("/api/convert", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (conversionType === "ocr-pdf") {
      console.log(
        "[OCR] Received OCR request for file:",
        req.file.originalname,
      );
    ...
    if (inputPath) {
      await fs
        .unlink(inputPath)
        .catch((err) =>
          console.error("[OCR] Failed to delete input file:", err),
        );
    }
    if (outputPath) {
      await fs
        .unlink(outputPath)
        .catch((err) =>
          console.error("[OCR] Failed to delete output file:", err),
        );
    }
  }
});
  ...
  return new Promise((resolve) => {
    ...
    pythonProcess.on("timeout", () => {
      ...
      resolve({
        success: false,
        error:
          "OCR processing timed out. Please try with a smaller file or different settings.",
      });
    });
  });
}
...
async function organizePdfPages(inputPath, outputPath, pageOperations) {
  return new Promise((resolve) => {
    const pythonProcess = spawn(
      "python3",
      ["pdf_converter.py", "organize_pdf", inputPath, outputPath],
      {
        timeout: 240000, // 4 minutes timeout
      },
    );
    ...
    pythonProcess.on("close", (code) => {
      if (code === 0) {
        ...
        if (
          output.includes("Validation error:") ||
          output.includes("Validation errors:")
        ) {
          ...
          const errorMessage =
            errorOutput || output || "Failed to organize PDF";
          resolve({ success: false, error: errorMessage });
        }
      }
    });
    ...
    pythonProcess.on("timeout", () => {
      ...
      resolve({
        success: false,
        error: "PDF organization timed out. Please try with a smaller file.",
      });
    });
  });
}
...
async function getPdfPageCount(inputPath) {
  return new Promise((resolve) => {
    const pythonProcess = spawn(
      "python3",
      ["pdf_converter.py", "get_page_count", inputPath],
      {
    ...
    pythonProcess.on("close", (code) => {
      if (code === 0) {
        ...
        const errorMessage =
          errorOutput || output || "Failed to get page count";
        resolve({ success: false, error: errorMessage });
      }
    });
    ...
    pythonProcess.on("timeout", () => {
      ...
      resolve({
        success: false,
        error:
          "Page count retrieval timed out. Please try with a smaller file.",
      });
    });
  });
}
